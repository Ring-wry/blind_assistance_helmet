...
LlamaCppNode::LlamaCppNode(const std::string &node_name,
  ...
  // 更新配置，declare_parameter和get_parameter成对出现，前者声明参数并设置默认值，后者获取参数值
  this->declare_parameter<int>("feed_type", feed_type_);    //数据来源, 0：VLM 本地（图片测试）；1：VLM 订阅（摄像头）; 2: LLM 订阅（纯文本大模型对话）
  this->declare_parameter<int>("model_type", model_type_);  //模型类型，0：internvl；1：smolvlm
  this->declare_parameter<std::string>("image", image_file_);   //本地图片路径,只有feed_type为0时有效
  this->declare_parameter<int>("is_shared_mem_sub", is_shared_mem_sub_);    //是否使用共享内存订阅图片, 只有feed_type为1时有效，1：使用共享内存订阅图片；0：使用ros topic订阅图片
  this->declare_parameter<int>("llm_threads", llm_threads_);    //大模型推理线程数
  this->declare_parameter<std::string>("llm_model_name", llm_model_name_);  //GGUF大模型名称，Qwen2.5-0.5B-Instruct-Q4_0.gguf
  this->declare_parameter<std::string>("model_file_name", model_file_name_);    //视觉模型文件路径，默认为vit_model_int16_v2.bin
  this->declare_parameter<std::string>("cute_words", cute_words_);  //启动的时候的开场白
  this->declare_parameter<std::string>("user_prompt", user_prompt_);  //默认用户提问，如果不发/prompt话题时，默认使用这个提问进行推理
  this->declare_parameter<std::string>("system_prompt", system_prompt_); //系统提示语，提供给大模型的上下文信息
  this->declare_parameter<int>("pre_infer", pre_infer_); //是否预推理，1：预推理，等待用户提问后再进行正式推理；0：不预推理，收到图片就进行正式推理
  this->declare_parameter<std::string>("ai_msg_pub_topic_name",
                                       ai_msg_pub_topic_name_); //发布的AI消息的话题名称，默认/llama_cpp_node
  this->declare_parameter<std::string>("text_msg_pub_topic_name",
                                      text_msg_pub_topic_name_); //发布的文本消息的话题名称，默认/tts_text，TTS语音播报订阅该话题
  this->declare_parameter<std::string>("ros_img_sub_topic_name",
                                       ros_img_sub_topic_name_); //订阅图片的话题名，默认/image
  this->declare_parameter<std::string>("ros_string_sub_topic_name",
                                       ros_string_sub_topic_name_); //订阅字符串的话题名，默认/prompt_text
  ...
  //判断是VLM还是LLM，如果是VLM则需要加载视觉模型，如果是LLM则不需要加载视觉模型
  ...
  //订阅话题模式，1：订阅图片话题进行推理，2：订阅文本话题进行推理
  else if (1 == feed_type_ || 2 == feed_type_) {
    RCLCPP_WARN(rclcpp::get_logger("llama_cpp_node"),
    "Create string subscription with topic_name: %s",
    ros_string_sub_topic_name_.c_str()); //创建订阅者，订阅用户提问文本话题
    ros_string_subscription_ =
        this->create_subscription<std_msgs::msg::String>(
            ros_string_sub_topic_name_,
            10,
            std::bind(
                &LlamaCppNode::RosStringProcess, this, std::placeholders::_1)); //回调函数RosStringProcess处理用户提问文本
    if (1 == feed_type_) {
      // 创建图片消息的订阅者
      RCLCPP_INFO(rclcpp::get_logger("llama_cpp_node"),
